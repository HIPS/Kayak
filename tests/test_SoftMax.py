...
def test_softmax_values_2():
    ...
    for ii in xrange(NUM_TRIALS):
        ...
        np_X = npr.randn(5,6)
        X    = kayak.Parameter(np_X)
        Y    = kayak.SoftMax(X, axis=0)

        np_Y = np.exp(np_X)
        np_Y = np_Y / np.sum(np_Y, axis=0, keepdims=True)
